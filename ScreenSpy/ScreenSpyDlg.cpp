...
BOOL CScreenSpyDlg::OnInitDialog()
{
	...
	//把对化框设置成全屏顶层窗口
	SetWindowPos(&wndTop/*wndTopMost*/, 0, 0, m_xScreen, m_yScreen, SWP_SHOWWINDOW);
	...
	//保存所有窗口位置
	m_CSCSpyWin.SnapshotAllWinRect();
	...
	m_ControlDlg.Create(IDD_CONTROL_DIALOG);
	m_ControlDlg.m_pScreenSpyDlg = this;
	//////////////////////////////////////////////////////////////////////////
	return TRUE; 
}
...
void CScreenSpyDlg::OnMouseMove(UINT nFlags, CPoint point) 
{
	   ...
	   if(m_bSelect)
	   {
		   m_rcSel[0].left=m_LBDownPoint.x;
		   m_rcSel[0].top = m_LBDownPoint.y;
		   m_rcSel[0].right = m_LBDownPoint.x;
		   m_rcSel[0].bottom = m_LBDownPoint.y;
	   ...
	   if (!m_bSelected && !m_bSelect)
	   {
		   m_rcSel[0] = m_CSCSpyWin.GetWinRectByPoint(point);
		   DrawFrame();
	   }
	   if(m_bSelected)
	   {
...
void CScreenSpyDlg::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	DrawFrame();
	//设置截图视频流的callback
	m_ControlDlg.StartCaptureStream(m_rcSel[0]);

	//把对化框设置成全屏顶层窗口
	SetWindowPos(&wndBottom/*wndTopMost*/, 0, 0, m_xScreen, m_yScreen, SWP_SHOWWINDOW);
	this->ShowWindow(SW_HIDE);

	m_ControlDlg.ShowWindow(SW_SHOWNORMAL);
...
void CScreenSpyDlg::ResetSelectROI()
{
	m_bSelect = false;
	m_bSelected = false;
	m_bLeftDown = false;
